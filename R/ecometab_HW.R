######Here we have the github version modified with new name, and calling f_calcWan09 for A-S flux. Will be called from WtReg by setting 
# metab_obs <- ecometab(wtreg_res_obs, DO_var = 'DO_obs', tz = tz, lat = lat, long = long, depth_vec = 4.4, replacemet = TRUE, gasex = 'Wanninkhof', gasave = 'instant'). So here H will be
# calculated using 4.4, the depth_vec. It will call Wanninkhof option, which will then call f_calcWan09 instead of the original f_calcWanninkhof. Other than that un touched. hopefully it will do A-S as instantaneious
#' Ecosystem metabolism ecometab.R slightly modified at all from github direct
#'
...
#' @param depth_vec single numeric value for manual entry of station depth (m).
...
#' removing instant = FALSE from list of conditions in following ecometabdefault. Instant there prevented instant later from being understood
#' removing was apparently bad idea as program quit  - saying it needed instant and it couldn't be found. So back in
#' removing .default as that force it to be an S3 method dispatch.
ecometab_HW <- function(dat_in, tz, DO_var = 'DO_mgl', metab_units = 'mmol', bott_stat = FALSE,
  depth_vec, replacemet = TRUE, instant = FALSE, gasex = c('Thiebault', 'Wanninkhof'), gasave = c('instant', 'daily', 'all'), ...){
  ...
  dat_in$DO<-dat_in[, DO_var, drop = T]/32*1000
  ...
  if(gasex == 'Wanninkhof')
    KL <- with(dat_in, f_calcWan09(Temp, Sal, WSpd))
