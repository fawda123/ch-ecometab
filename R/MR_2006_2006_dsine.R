# ANalysis of sine efffectg in wtreg
# PIE_MR_2006_b.R 
...
out_dir <- normalizePath("c:/Users/chopkins/Documents/R_Projects/Metab/MR_2006_WtReg_Results",
                         winslash = "/", mustWork = TRUE)
setwd(out_dir)

# ---- Read input & prep ----
data_file <- file.path(out_dir, "MR_2006.csv")   # <-- change filename as needed
...
# ---- Window grids ----
w1_values <- c(.1, 3.075, 6.05, 9.025, 12)
w2_values <- c(.1, 3.075, 6.05, 9.025, 12)
w3_values <- seq(0.1, 1.0, length.out = 5)

# ---- Outputs ----
log_path     <- file.path(out_dir, "MR_2006_sine_metabolism_model_log.txt")
summary_path <- file.path(out_dir, "MR_2006_sine_summary_metabolism_results.csv")
...
wtreg_res_obs <- wtreg(IBYC_nonan,
                       wins = list(w1_values[1], w2_values[1], w3_values[1]),
                       progress = FALSE, tz = tz, lat = lat, long = long,
                       parallel = FALSE, sine = TRUE)  # turn off internal parallelism and incorporate sine transformation

metab_obs <- ecometab(wtreg_res_obs, DO_var = 'DO_obs', tz = tz, lat = lat, long = long)

# Save for reference
readr::write_csv(metab_obs, file.path(out_dir, "MR_2006_sine_metab_obs_reference.csv"))
...
process_combo <- function(w1, w2, w3) {
  ...
  tryCatch({
    ...
    wtreg_res <- wtreg(IBYC_nonan, wins = list(w1, w2, w3),
                       progress = FALSE, tz = tz, lat = lat, long = long,
                       parallel = FALSE)  # keep parallelism only at the furrr layer
    
    metab_dtd <- ecometab(wtreg_res, DO_var = 'DO_nrm', tz = tz, lat = lat, long = long)
    
    # Save full outputs (absolute paths => safe in parallel workers)
    readr::write_csv(wtreg_res, file.path(out_dir, paste0("wtreg_res_MR2006_sine_", combo_tag, ".csv")))
    readr::write_csv(metab_dtd, file.path(out_dir, paste0("metab_dtd_MR2006_sine_", combo_tag, ".csv")))
    ...
    if (is.null(rawdat) || is.null(depth_val) || is.null(DO_var)) {
      ...
      readr::write_csv(mos, file.path(out_dir, paste0("MR_2006_sinemonthly_eval_", combo_tag, ".csv")))
